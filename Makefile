...
.PHONY: node
node:
	docker run --rm --entrypoint bash -i -v $(shell pwd)/:/var/www/html -w /var/www/html/frontend wodby/node:$(NODE_TAG) -c "yarn install && yarn build && rm -rf frontend/node_modules"
